...
const MoodSelectModal = ({
  ...
  return (
    <BaseModal isOpen={isOpen} onClose={onClose}>
      <div className="p-2 sm:p-6 w-full max-w-[1000px] mx-auto overflow-x-hidden max-h-[80vh] overflow-y-auto">
        {!isDirectSelect && (
              ...
              <div className="flex flex-wrap justify-center gap-2 sm:gap-2.5 md:gap-3 mb-3">
                {analyzedKeywords.map((keyword, index) => (
                  <button
                    ...
                    className={`min-w-[100px] px-3 py-1.5 sm:px-3 sm:py-1.5 md:px-4 md:py-2 border rounded-lg transition-all duration-200 hover:-translate-y-0.5 cursor-pointer text-center break-words max-w-full
        ...
        <div className="flex justify-end items-center gap-3 relative">
          {!isAnalysisFailed && !isDirectSelect && (
            <button
              ...
              className="cursor-pointer text-sm text-[#4A7196] underline hover:text-[#3A5A7A] transition-colors"
            >
              추천 감정이 내 마음과 다르다면? 직접 골라보세요!
            </button>
          )}
          ...
          <button
            ...
            className="cursor-pointer px-6 sm:px-7 md:px-8 py-2 sm:py-2.5 bg-[#4A7196] text-white rounded-full hover:bg-[#3A5A7A] 
