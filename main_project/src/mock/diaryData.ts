import { Diary } from "../models/diary";
import { Mood } from "../models/diary";

export const mockDiaries: Diary[] = [
  {
    diary_id: 1,
    date: "2025-03-25",
    title: "봄날의 산책",
    content:
      "오늘은 날씨가 너무 좋아서 한강공원에 산책을 다녀왔다. 벚꽃이 피기 시작하는 모습이 정말 예뻤다. 봄이 오는 것을 온몸으로 느낄 수 있었다.",
    created_at: "2025-03-25T14:30:00",
    moods: [Mood.Happiness, Mood.Excitement],
    rec_music: {
      video_id: "1",
      title: "봄날",
      artist: "BTS",
      thumbnail: "",
      embedUrl: "",
    },
  },
  {
    diary_id: 2,
    date: "2025-03-24",
    title: "일요일의 휴식",
    content:
      "집에서 넷플릭스 보면서 편안하게 쉬었다. 가끔은 이렇게 아무것도 하지 않는 것도 좋은 것 같다.",
    created_at: "2025-03-24T20:00:00",
    moods: [Mood.Satisfaction, Mood.Relief],
    rec_music: {
      video_id: "2",
      title: "Sunday Morning",
      artist: "Maroon 5",
      thumbnail: "",
      embedUrl: "",
    },
  },
  {
    diary_id: 3,
    date: "2025-03-23",
    title: "친구들과의 모임",
    content:
      "오랜만에 대학교 친구들을 만났다. 다들 각자의 자리에서 열심히 살고 있어서 보기 좋았다. 우리도 어느덧 다 어른이 되었네.",
    created_at: "2025-03-23T19:00:00",
    moods: [Mood.Excitement, Mood.Nostalgia, Mood.Happiness],
    rec_music: {
      video_id: "3",
      title: "우리가 맞이할 새로운 시작",
      artist: "아이유",
      thumbnail: "",
      embedUrl: "",
    },
  },
  {
    diary_id: 4,
    date: "2025-03-22",
    title: "업무 스트레스",
    content:
      "프로젝트 데드라인이 다가오는데 아직 해결하지 못한 문제들이 많다. 주말에도 일해야 할 것 같아 걱정이다.",
    created_at: "2025-03-22T22:15:00",
    moods: [Mood.Anxiety, Mood.Frustration],
    rec_music: {
      video_id: "4",
      title: "힘내",
      artist: "소녀시대",
      thumbnail: "",
      embedUrl: "",
    },
  },
  {
    diary_id: 5,
    date: "2025-03-28",
    title: "장문일기 테스트",
    content:
      "오늘은 아침부터 정신없이 바쁜 하루였다. 눈을 뜨자마자 미팅 준비를 해야 했고, 급하게 커피 한 잔을 내리면서 노트북을 켰다. 오전 회의는 예상보다 길어졌고, 끝나자마자 바로 다음 업무에 몰입해야 했다. 집중해서 작업을 하다 보니 점심시간을 놓쳐버려 겨우 간단한 샌드위치로 때웠다.그렇게 하루를 보내다 보니 저녁이 되어서야 한숨 돌릴 수 있었다. 문득 창밖을 보니 노을이 아름답게 물들어 있었다. 하루 종일 바쁘게 움직였지만, 이 순간만큼은 모든 걸 잠시 멈추고 창가에 기대어 하늘을 바라보았다. 핸드폰을 내려놓고, 조용한 음악을 틀어놓으니 마음이 차분해지는 기분이었다.바쁜 일상 속에서도 이런 작은 여유가 얼마나 소중한지 다시금 느꼈다. 앞으로는 하루 중 잠깐이라도 나를 위한 시간을 꼭 가져야겠다고 다짐했다. 내일도 바쁘겠지만, 오늘처럼 작은 여유를 찾으며 하루를 마무리하고 싶다.",
    created_at: "2025-03-28T18:30:00",
    moods: [Mood.Happiness, Mood.Confidence],
    rec_music: {
      video_id: "5",
      title: "Swim Good",
      artist: "Frank Ocean",
      thumbnail: "",
      embedUrl: "",
    },
  },
  {
    diary_id: 6,
    date: "2025-03-20",
    title: "오늘의 일기",
    content: "오늘은 정말 좋은 날씨였다. 공원에서 산책을 하고 커피를 마시며 즐거운 시간을 보냈다.",
    created_at: "2025-03-20T10:00:00",
    moods: [Mood.Happiness, Mood.Satisfaction],
    rec_music: {
      video_id: "1",
      title: "행복한 노래",
      artist: "가수",
      thumbnail: "",
      embedUrl: "",
    },
  },
  {
    diary_id: 7,
    date: "2025-03-19",
    title: "힘든 하루",
    content: "오늘은 업무가 많아서 스트레스를 많이 받았다. 하지만 이겨낼 수 있을 거야.",
    created_at: "2025-03-19T20:30:00",
    moods: [Mood.Anxiety, Mood.Hope],
    rec_music: {
      video_id: "2",
      title: "힐링 음악",
      artist: "힐링가수",
      thumbnail: "",
      embedUrl: "",
    },
  },
  {
    diary_id: 8,
    date: "2025-03-18",
    title: "즐거운 주말",
    content: "친구들과 함께 영화를 보고 맛있는 음식을 먹었다. 정말 즐거운 하루였다.",
    created_at: "2025-03-18T15:45:00",
    moods: [Mood.Excitement, Mood.Happiness, Mood.Satisfaction],
    rec_music: {
      video_id: "3",
      title: "즐거운 노래",
      artist: "즐거운가수",
      thumbnail: "",
      embedUrl: "",
    },
  },
  {
    diary_id: 9,
    date: "2025-03-17",
    title: "평온한 하루",
    content: "집에서 책을 읽고 음악을 들으며 평화로운 시간을 보냈다.",
    created_at: "2025-03-17T14:20:00",
    moods: [Mood.Satisfaction, Mood.Relief],
    rec_music: {
      video_id: "4",
      title: "평화로운 음악",
      artist: "평화로운가수",
      thumbnail: "",
      embedUrl: "",
    },
  },
  {
    diary_id: 10,
    date: "2025-03-16",
    title: "우울한 날",
    content: "오늘은 왠지 우울한 기분이 든다. 하지만 이 또한 지나갈 거야.",
    created_at: "2025-03-16T21:15:00",
    moods: [Mood.Depression, Mood.Hope],
    rec_music: {
      video_id: "5",
      title: "위로의 노래",
      artist: "위로가수",
      thumbnail: "",
      embedUrl: "",
    },
  },
  {
    diary_id: 11,
    date: "2025-04-15",
    title: "봄비의 하루",
    content:
      "오늘은 봄비가 내렸다. 창밖의 빗소리를 들으며 책을 읽었다. 비가 내리는 소리가 마음에 평화를 가져다주었다.",
    created_at: "2025-04-15T15:30:00",
    moods: [Mood.Satisfaction, Mood.Emotion],
    rec_music: {
      video_id: "6",
      title: "Rain",
      artist: "태연",
      thumbnail: "",
      embedUrl: "",
    },
  },
  {
    diary_id: 12,
    date: "2025-04-10",
    title: "새로운 시작",
    content: "새로운 직장에서의 첫 날이었다. 긴장되기도 했지만, 새로운 환경에서의 도전이 기대된다.",
    created_at: "2025-04-10T20:00:00",
    moods: [Mood.Excitement, Mood.Tension, Mood.Hope],
    rec_music: {
      video_id: "7",
      title: "New Beginning",
      artist: "아이유",
      thumbnail: "",
      embedUrl: "",
    },
  },
  {
    diary_id: 13,
    date: "2025-04-05",
    title: "벚꽃 구경",
    content:
      "친구들과 함께 벚꽃 구경을 다녀왔다. 예쁜 벚꽃과 함께 사진도 많이 찍었다. 봄의 시작이 정말 좋다.",
    created_at: "2025-04-05T14:20:00",
    moods: [Mood.Happiness, Mood.Excitement, Mood.Emotion],
    rec_music: {
      video_id: "8",
      title: "Cherry Blossom",
      artist: "AKMU",
      thumbnail: "",
      embedUrl: "",
    },
  },
  {
    diary_id: 14,
    date: "2025-04-01",
    title: "4월의 시작",
    content: "4월이 시작되었다. 이번 달에는 새로운 목표를 세우고 달성해보자.",
    created_at: "2025-04-01T09:00:00",
    moods: [Mood.Happiness, Mood.Hope],
    rec_music: {
      video_id: "9",
      title: "April",
      artist: "BTS",
      thumbnail: "",
      embedUrl: "",
    },
  },
  {
    diary_id: 15,
    date: "2025-04-20",
    title: "힘든 하루",
    content: "오늘은 업무가 많아서 스트레스를 많이 받았다. 하지만 이겨낼 수 있을 거야.",
    created_at: "2025-04-20T22:15:00",
    moods: [Mood.Anxiety, Mood.Hope],
    rec_music: {
      video_id: "10",
      title: "힘내",
      artist: "소녀시대",
      thumbnail: "",
      embedUrl: "",
    },
  },
];
